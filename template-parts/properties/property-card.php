...
<article class="property-card group bg-white  overflow-hidden border border-slate-100 shadow-sm hover:shadow-2xl transition-all duration-500 flex flex-col h-full" data-aos="fade-up">
        ...
        <div class="absolute bottom-6 left-2 right-2">
            <div class="bg-white/90 backdrop-blur-md py-3 px-1.5  shadow-lg border border-white/20">
                <div class="flex justify-between items-center gap-2">
                    ...
                    <div class="text-end text-[11px] text-slate-500! uppercase font-medium tracking-widest w-[70%]">
    ...
    <div class="py-4 px-4 flex flex-col flex-1">
        ...
        <div class="mt-auto">
            <?php 
            $detail_slug = ($property['is_investment'] ?? false) ? '/investment-details/' : '/property-details/';
            ?>
            <a href="<?php echo esc_url( home_url( $detail_slug . '?id=' . urlencode($property['id'] ?? '') ) ); ?>" class="flex items-center justify-center gap-3 w-full bg-slate-900 text-white font-black text-xs uppercase tracking-[0.2em] py-4  hover:bg-primary transition-all duration-300 shadow-xl shadow-slate-900/10 active:scale-[0.98]">
